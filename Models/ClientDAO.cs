...
namespace LabMVC.Models
{
    class ClientDAO
    {
        ...
        //private string cnStr = "Data Source=(local);Initial Catalog=Users;Integrated Security=True";
        private string cnStr = "Data Source=ALHASSANE1\\SQLEXPRESS;Initial Catalog=Lab_MVC;Integrated Security=True";
        /// <summary>
        /// Objet de factory.
        /// </summary>
        ...
        public int SauvegardeClient(ClientVO clientvo)
        {
            ...
            cmdCommand.CommandText = "Insert Into tb_Client (Prenom,Nom,DateNaissance,Description,Adresse,Telephone) Values('" + clientvo.PrenomClient + "','" + clientvo.NomClient + "','" + clientvo.DateNaissanceClient + "','" + clientvo.DescriptionClient + "','" + clientvo.AdresseClient + "', '" + clientvo.TelephoneClient + "');";
            using (DbDataReader dr = cmdCommand.ExecuteReader()) ;

                //Retrouver le numeroClient
                int idTrouve = 0;
            cmdCommand.CommandText = "SELECT TOP 1 * FROM tb_Client ORDER BY ID DESC";
            using (DbDataReader dr = cmdCommand.ExecuteReader())
            {
                // Lecture de tous les résultats.
                while (dr.Read())
                {
                    idTrouve = int.Parse(dr["ID"].ToString());
                }
            }
            return idTrouve;
        }
